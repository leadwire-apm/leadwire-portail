...
class MonitoringSetFixture extends AbstractFixture implements OrderedFixtureInterface
{
    ...
    public function load(ObjectManager $manager)
    {
        ...
        $ms->setName("APM")->setQualifier("APM")->setVersion("7.3.2")->addToApplicationType($applicationType);
        $manager->persist($ms);
        $this->addReference(self::APM_MONITORING_SET, $ms);
        $ms = new MonitoringSet();
        $ms->setName("Metricbeat")->setQualifier("METRICBEAT")->setVersion("7.3.2")->addToApplicationType($applicationType);
        $manager->persist($ms);
